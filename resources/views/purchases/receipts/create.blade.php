                ...
                @foreach($order->lines as $index => $line)
                    ...
                    @if(\App\Domain\Inventory\InventoryQuantity::isPositive($balance['remaining']))
                        <article class="sulu-card p-5" x-data="{ selected: {{ $selected ? 'true' : 'false' }} }">
                            <div class="flex flex-wrap items-start justify-between gap-4"><div><p class="text-sm font-bold text-white">{{ $line->product->sku }} · {{ $line->description }}</p><p class="mt-1 text-xs text-slate-500">Pedido {{ $line->ordered_quantity }} · recibido {{ $balance['received'] }} · pendiente {{ $balance['remaining'] }} {{ $line->base_unit_code }}</p></div><label class="flex items-center gap-2 text-sm font-semibold text-emerald-200"><input type="checkbox" x-model="selected" class="rounded border-slate-600 bg-slate-900 text-emerald-400 focus:ring-emerald-400"> Incluir</label></div>
                            <input type="hidden" name="lines[{{ $index }}][purchase_order_line_id]" value="{{ $line->id }}" :disabled="!selected">
                            <div class="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                                <div><label class="text-xs font-semibold text-slate-400">Cantidad</label><input name="lines[{{ $index }}][quantity]" inputmode="decimal" value="{{ $old['quantity'] ?? $balance['remaining'] }}" :disabled="!selected" class="mt-2 w-full rounded-xl border-slate-700 bg-slate-950 font-mono text-slate-100 disabled:opacity-40 focus:border-emerald-400 focus:ring-emerald-400"></div>
