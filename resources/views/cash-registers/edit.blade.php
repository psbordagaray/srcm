<x-app-layout>
    <div class="mx-auto max-w-3xl space-y-6">
        <div>
            ...
            <h1 class="mt-2 text-2xl font-bold text-white">Editar {{ $register->name }}</h1>
        </div>
        ...
        <form method="POST" action="{{ route('cash-registers.update', $register) }}" class="sulu-card space-y-5 p-6">
            @csrf
            @method('PUT')
            ...
            <label class="block">
                ...
                >
                    @foreach($accounts as $account)
                        <option
                            value="{{ $account->id }}"
                            @selected((string) old('financial_account_id', $register->financial_account_id) === (string) $account->id)
                        >
            ...
            <p class="text-xs text-slate-500">
                Después del primer turno, la cuenta financiera de la caja queda fijada para preservar trazabilidad.
            </p>
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit" class="rounded-xl bg-amber-400 px-5 py-2 text-sm font-black text-slate-950">
                    Guardar
                </button>
            </div>
        </form>
