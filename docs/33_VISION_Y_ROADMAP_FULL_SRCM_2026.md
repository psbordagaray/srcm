...
## Current P12 continuity — P12.2 browser-bound operational device foundation published
...
P12 has now established both server-side operational-device identity and a browser binding boundary. Functional checkpoint `889823a566a60ca9ff10aad82547cc6d43156b30` (`feat(offline): add operational device browser binding`) is published from parent `5fed76aab34dde60d2241d4ca95db624cecebb74` with tree `6a0222e4ebceba14615ac2d3fca85e3b017c428f`; CI65 run `33200967493` and `quality-gates` job `98950027992` completed successfully on the exact feature SHA.

The architectural truth now spans two layers:
- P12.1: opaque tenant-scoped operational-device UUID, vendor-neutral capabilities, and immutable/idempotent operation-claim ledger;
- P12.2: a server-issued browser credential that binds an authenticated browser session to an operational device without treating the public device UUID as a secret.

Browser Binding contract:
- 256-bit random token generated by the server;
- only the SHA-256 token hash persists;
- plaintext token is delivered only through an `HttpOnly`, `SameSite=Strict` cookie;
- production/HTTPS requires secure-cookie transport;
- bindings expire, can be revoked, and rotate prior live credentials for the same device;
- resolution fails closed for wrong tenant, revoked/expired binding or inactive device;
- browser identity and human authority remain separate dimensions;
- the runtime endpoint is read-only and intentionally exposes no token, hash or mutation authority.

The canonical local schema now has **124 migrations**, including `operational_device_browser_bindings`. Post-push reconciliation verified the table present with zero real bindings, repository clean, `.env` unchanged, protected main unchanged and CI65 GREEN.

This is still **not** an offline transaction engine. Service Worker, IndexedDB/local persistence, offline mutation queues, offline sale finalization, offline payment finalization, offline fiscal authorization, peripheral drivers, transactional kiosk and RFID/EAS remain separate boundaries. Silent reconciliation of stale price or stock is explicitly forbidden.

The next exact boundary is:
`P12_2_RESTRICTED_OFFLINE_READ_MODEL_SNAPSHOT_RECON_V1`.

That boundary must be read-only first. It will define a versioned, server-authoritative client snapshot contract: which catalog/pricing/availability/configuration facts may travel to a bound device, their freshness and invalidation semantics, size/privacy constraints and the exact authority rules that apply when connectivity returns. Only after this contract is proven should browser persistence be introduced.

The North Star remains restricted offline continuity: the system may preserve useful operation during connectivity loss only where authority, evidence and conflict semantics remain explicit. It must never convert stale local data into a silent final commercial, payment or fiscal truth.
