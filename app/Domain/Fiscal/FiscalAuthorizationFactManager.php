...
final class FiscalAuthorizationFactManager
{
    ...
    public function record(FiscalAuthorizationFactData $data, User $actor): FiscalAuthorizationAttempt
    {
        ...
        return DB::transaction(function () use ($data, $actor, $organizationId): FiscalAuthorizationAttempt {
            ...
            $existing = FiscalAuthorizationAttempt::query()
                ->forOrganization($organizationId)
            ...
            if ($existing) {
                ...
                return $existing->load('response');
            }

            if (
                $data->outcome === FiscalAuthorizationOutcome::Authorized
                && $document->authorizationAttempts()
                    ->whereHas(
                        'response',
                        fn ($query) => $query->where(
                            'outcome',
                            FiscalAuthorizationOutcome::Authorized->value
                        )
                    )
                    ->exists()
            ) {
            ...
            $attempt = FiscalAuthorizationAttempt::query()->create([
            ...
            FiscalAuthorizationResponse::query()->create([
                'organization_id' => $organizationId,
                'fiscal_authorization_attempt_id' => $attempt->id,
    ...
    {
        $id = $this->currentOrganization->id($actor);
